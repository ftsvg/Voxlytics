...
class LevelLeaderboardUpdates(commands.Cog):
    def __init__(self, client: commands.Bot):
        self.client = client
        self.db = LeaderboardHandler("level")
        self.update_message.start()

    @tasks.loop(minutes=1)
    async def update_message(self):
        try:
            latest = await LeaderboardInfo.fetch_leaderboard("level", num=100)
            if not latest:
                return
            ...
            for uuid, old_pos, new_pos in updates:
                ...
                embed.set_author(name="Level Leaderboard Update", icon_url=skin_head)
            ...
            logger.exception(
                f"Unhandled exception in Level Leaderboard Updates task: {error}"
            )
...
async def setup(client: commands.Bot):
    await client.add_cog(LevelLeaderboardUpdates(client))
